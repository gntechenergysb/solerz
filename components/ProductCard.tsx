    ...
    <Link to={`/listing/${listing.id}`} className="group block h-full">
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 h-full flex flex-col overflow-hidden relative">
        ...
        <div className="p-4 flex-grow flex flex-col">
          <div className="mb-2">
            <div className="text-xs font-semibold text-emerald-600 mb-0.5 uppercase tracking-wider">{categoryLabel}</div>
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 leading-snug line-clamp-2 group-hover:text-emerald-600 transition-colors">
              {listing.title}
            </h3>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {getSpecsTags().map((tag, idx) => (
              <span key={idx} className="inline-flex items-center bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-200 text-[11px] font-medium px-2 py-0.5 rounded-md border border-slate-200 dark:border-slate-700">
                {tag}
              </span>
            ))}
