...
const HandbookPage: React.FC = () => {
  ...
  useEffect(() => {
    document.title = 'Solar PV Engineering Handbook & Field Design Formulas | Solerz';

    // Inject FAQPage Schema for AI Engine Optimization
    const aeoSchema = {
      '@context': 'https://schema.org',
      '@graph': [
        {
          '@type': 'FAQPage',
          name: 'Solar Engineering Calculation Guides and Design Rules',
          mainEntity: SOLAR_TIPS.map((tip) => ({
            '@type': 'Question',
            name: tip.question,
            acceptedAnswer: {
              '@type': 'Answer',
              text: `${tip.summary} Formula: ${tip.formulaOrRule}.`,
            },
          })),
        },
        {
          '@type': 'DefinedTermSet',
          name: 'Solar Photovoltaic & Energy Storage Engineering Standards',
  ...
  // Tip categories
  const tipCategories = [
    { key: 'all', label: 'All Guides', icon: <Sparkles className="w-3.5 h-3.5" /> },
  ...
  const standardCategories = [
    { key: 'all', label: 'All Codes', icon: <Sparkles className="w-3.5 h-3.5" /> },
    ...
    { key: 'installation', label: 'Installation & Codes', icon: <ShieldCheck className="w-3.5 h-3.5" /> },
  ];
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-16">
      ...
      <div className="text-center space-y-3 pt-2">
        <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200/60 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-semibold uppercase tracking-wider">
          ...
          Engineering Reference &amp; Design Formulas
        </div>
        ...
        <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 max-w-2xl mx-auto leading-relaxed">
          Quick-reference calculation formulas, electrical rules of thumb, and field design principles for photovoltaic and energy storage systems.
        </p>
      </div>
      ...
      <div className="flex items-center justify-center">
        <div className="p-1 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm inline-flex gap-1">
          <button
            type="button"
            onClick={() => {
              setViewMode('tips');
            ...
            className={`inline-flex items-center gap-2 px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              viewMode === 'tips'
          ...
          >
            ...
            Engineering Guides &amp; Formulas ({SOLAR_TIPS.length})
          </button>
          ...
          >
            ...
            Compliance Codes ({SOLAR_STANDARDS.length})
          </button>
        </div>
      </div>
      ...
      <div className="sticky top-14 sm:top-16 z-20 bg-slate-50/90 dark:bg-slate-950/90 backdrop-blur-md py-3 space-y-3">
        ...
        <div className="relative max-w-xl mx-auto">
          ...
          <input
            ...
            placeholder={
              viewMode === 'tips'
                ? 'Search formulas, MPPT, bifacial, cold Voc, temperature derating...'
                : 'Search codes: IEC 61215, UL 9540, IEEE 1547, Rapid Shutdown...'
            }
            value={searchQuery}
      ...
      {/* ----------------------------------------------------------------- */}
      {/* 5. VIEW B: Compliance Codes Directory Grid */}
      {/* ----------------------------------------------------------------- */}
      {viewMode === 'standards' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filteredStandards.map((std) => (
            ...
            >
              <div className="space-y-3">
                {/* Standard Code, Issuing Body, and Status */}
      ...
      {((viewMode === 'tips' && filteredTips.length === 0) ||
        ...
        <div className="text-center py-16 space-y-3">
          ...
          <p className="text-xs text-slate-500">
            Try searching for terms like "MPPT", "Bifacial", "Cold Voc", "IEC 61215", or "Rapid Shutdown".
          </p>
        </div>
      )}
    </div>
