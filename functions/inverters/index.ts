...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const canonical = `${origin}/inverters`;

  let baseHtml = await fetchIndexHtml(env, origin);
  baseHtml = cleanBaseHtml(baseHtml);

  const title = 'Solar Inverters Database & Technical Specifications | Solerz';
  const description =
    'Browse certified string, micro, and central solar inverters. Sandia efficiency curves, MPPT voltage ranges, AC power output, and battery storage compatibility parameters on Solerz.';
  ...
  const jsonLd = {
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, item: { '@id': canonical, name: 'Inverters' } },
      ],
    },
  };
  ...
  const prerenderBody = `
  <div id="ssr-inverters-index-prerender" style="max-width: 1100px; margin: 30px auto; padding: 0 20px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1e293b; line-height: 1.6;">
    <nav style="font-size: 13px; margin-bottom: 20px; color: #64748b;">
      <a href="/" style="color: #059669; text-decoration: none; font-weight: 600;">Home</a> &gt; <span>Solar Inverters</span>
    </nav>
    <header style="border-bottom: 2px solid #e2e8f0; padding-bottom: 20px; margin-bottom: 30px;">
      <div style="display: inline-block; padding: 4px 12px; background: #ecfdf5; color: #059669; font-size: 11px; font-weight: 700; border-radius: 9999px; text-transform: uppercase; margin-bottom: 8px;">
        Power Conversion Vault
      </div>
      <h1 style="font-size: 32px; font-weight: 900; color: #0f172a; margin: 0 0 10px 0;">
        Solar Inverters Database
      </h1>
      <p style="font-size: 16px; color: #475569; margin: 0;">
        Certified grid-tie, micro, and battery hybrid inverters. Continuous AC power, MPPT tracking channels, and Sandia efficiency models.
      </p>
    </header>
  </div>
  `;
